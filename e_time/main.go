	...
	for {
		...
		if n.isLeftChild() && n.parent.isLeftChild() {
			...
			if n.isRightChild() && n.parent.isRightChild() {
				...
				panic("assertion fails: should not get here on case 5.")
			}
		}
		break
	}
...
func (root *Tree) doDelete(n *node) {
	if n.myTree != nil && n.myTree != root {
		panic("delete applied to node that was not from our tree")
	}
...
func comp(a, b Item) int {
	...
	switch {
	case tt.T1.After(other.T1):
		return 1
	case tt.T1.Before(other.T1):
		return -1
	default:
		return 0
	}
	...
	for ; numSets > 0; numSets-- {
		...
		for ; numTimes > 0; numTimes-- {
			...
			if nil != err1 || nil != err2 {
				...
				if !t.Valid() || !knownTimes.Insert(t) {
					// fmt.Println("!!!wrong", t)
					valid = false
				}
